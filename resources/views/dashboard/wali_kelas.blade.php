                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <form method="GET" action="{{ route('dashboard.waliKelas') }}" id="form-mapel">
                                <input type="hidden" name="kelas_ajar_id"
                                    value="{{ optional($selectedKelasAjar)->kelas_ajar_id }}">
                                ...
                                <div class="d-flex gap-2">
                                    <select class="form-select" name="intrakurikuler_id"
                                        ...
                                        @forelse($mapels as $m)
                                            <option value="{{ $m->intrakurikuler_id }}"
                                                @selected(
                                                    optional($selectedMapel)->intrakurikuler_id == $m->intrakurikuler_id
                                                    || (
                                                        !$selectedMapel
                                                        && optional($topMapel)->intrakurikuler_id == $m->intrakurikuler_id
                                                    )
                                                )>
                                                {{ $m->nama_pelajaran }}
                                            </option>
                                    ...
                                    <button class="btn btn-primary" type="submit"
                                        @disabled(!$selectedKelasAjar || $mapels->isEmpty())>
                                        Terapkan
                                    </button>
                                </div>
        ...
        @else
            ...
            <div class="col-lg-7">
                <div class="card">
                    ...
                    <div class="card-body">
                        {{-- FIX: kasih min-height + overflow hidden biar gak nembus --}}
                            ...
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="2" class="text-muted text-center py-3">
                                                Tidak ada siswa di bawah 60 pada semester ini.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            ...
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="2" class="text-muted text-center py-3">
                                                Belum ada data nilai lintas mapel pada semester ini.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
...
@section('scripts')
    ...
    <script>
        ...
        // ===== FIX RENDER CHART (agar tidak "nembus" saat first load) =====
        ...
        function renderDistribusiChart() {
            ...
            // destroy dulu biar ga dobel / ukurannya ngaco
            ...
            chartDistribusiInstance.render().then(() => {
                // resize setelah render (buat kasus layout/sidebar belum stabil)
                setTimeout(() => {
        ...
        // Render saat DOM siap
        document.addEventListener('DOMContentLoaded', () => {
            setTimeout(renderDistribusiChart, 50);
        });

        window.addEventListener('load', () => {
            setTimeout(renderDistribusiChart, 0);
        });
