        ...
        <div class="card">
            ...
            <div class="card-header d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-2">
                <div>
                    ...
                    {{-- model intrakurikuler relasi: kelasAjar -> tahunAjaran --}}
                        ...
                        <tbody>
                            @forelse ($intrakurikuler as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->kelasAjar?->tahunAjaran?->tahun ?? '-' }}</td>
                                    <td>{{ ucfirst($item->kelasAjar?->tahunAjaran?->semester ?? '-') }}</td>
                                    <td>{{ $item->kelasAjar?->kelas?->nama_kelas ?? '-' }}</td>

                                    @role('Bagian Akademik')
                                        <td>{{ $item->pengampu?->staff?->nama ?? ($item->pengampu?->name ?? '-') }}</td>
                                    @endrole
                                    ...
                                    @role('Bagian Akademik|Super Admin|Guru Mapel')
                                        <td>
                                            ...
                                            <a href="{{ route('absensi.intrakurikuler.rekap', $item->intrakurikuler_id) }}">
